...
def selTournamentDCD(individuals, k):
    ...
    be a multiple of 4 only if k is equal to the length of individuals. It that is not the case, the function falls
    ...
    if k == len(individuals) and k % 4 != 0:
        return selTournamentDCDunrestricted(individuals, k)
...
def selTournamentDCDunrestricted(individuals, k):
    """Like selTournamentDCD() but without restrictions in size of k.
    Tournament selection based on dominance (D) between two individuals, if
    the two individuals do not interdominate the selection is made
    ...
    chosen = []
    for _ in range(k):
        chosen.append(tourn(random.choice(individuals_1),   random.choice(individuals_2)))

    return chosen


def selTournamentConstraint(individuals, k):
    """Tournament selection based on constraint satisfaction. The *individuals* sequence length has to
    be a multiple of 4 only if k is equal to the length of individuals. It that is not the case, the function falls
    back to another version.
    Starting from the beginning of the selected individuals, two consecutive
    individuals will be different (assuming all individuals in the input list
    ...
    This selection requires the individuals to have a :attr:`constraints`
    attribute.
    ...
    if k > len(individuals):
        raise ValueError("selTournamentConstraint: k must be less than or equal to individuals length")

    if k == len(individuals) and k % 4 != 0:
        return selTournamentConstraintunrestricted(individuals, k)

    def tourn(ind1, ind2):

        if sum([cons.value for cons in ind1.constraints]) < sum([cons.value for cons in ind2.constraints]):
            return ind1
        elif sum([cons.value for cons in ind1.constraints]) > sum([cons.value for cons in ind2.constraints]):
            return ind2
...
def selTournamentConstraintunrestricted(individuals, k):
    """Like selTournamentConstraint() but without restrictions in size of k.
    Tournament selection based on dominance (D) between two individuals, if
    the two individuals do not interdominate the selection is made
    based on crowding distance (CD).
    Starting from the beginning of the selected individuals, two consecutive
